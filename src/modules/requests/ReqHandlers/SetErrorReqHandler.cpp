...
ArgValidationResult SetErrorReqHandler::validateArgs(const std::vector<std::string> &args)
{
    if (args.empty())
    ...
    for (const auto &arg : args)
    {
        std::string cmd = std::string(AT_CMD_SET_ERROR) + AT_CMD_EQUAL + arg + AT_CMD_CARRIAGE_RETURN;
        exec_cmds.push(cmd);
    }
...
const std::string SetErrorReqHandler::prepareCommand(void)
{
...
bool SetErrorReqHandler::reqExecEnded(void)
{
    if (this->exec_cmds.empty())
    {
