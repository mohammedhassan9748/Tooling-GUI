...
ArgValidationResult ClearChannelReqHandler::validateArgs(const std::vector<std::string> &args)
{
    if (args.empty())
    ...
    for (const auto &arg : args)
    {
        std::string cmd = std::string(AT_CMD_CLEAR_CHANNEL) + arg + AT_CMD_CARRIAGE_RETURN;
        exec_cmds.push(cmd);
    }
...
const std::string ClearChannelReqHandler::prepareCommand(void)
{
...
bool ClearChannelReqHandler::reqExecEnded(void)
{
    if (this->exec_cmds.empty())
    {
