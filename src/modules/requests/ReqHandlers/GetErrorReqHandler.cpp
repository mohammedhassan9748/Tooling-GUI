...
ArgValidationResult GetErrorReqHandler::validateArgs(const std::vector<std::string> &args)
{
    if (args.empty())
    ...
    for (const auto &arg : args)
    {
        std::string cmd = std::string(AT_CMD_GET_ERROR) + AT_CMD_EQUAL + arg + AT_CMD_CARRIAGE_RETURN;
        exec_cmds.push(cmd);
    }
...
const std::string GetErrorReqHandler::prepareCommand(void)
{
...
bool GetErrorReqHandler::reqExecEnded(void)
{
    if (this->exec_cmds.empty())
    {
