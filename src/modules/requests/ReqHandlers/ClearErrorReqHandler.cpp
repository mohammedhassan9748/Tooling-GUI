...
ArgValidationResult ClearErrorReqHandler::validateArgs(const std::vector<std::string> &args)
{
    if (args.empty())
    ...
    for (const auto &arg : args)
    {
        ...
        if (value < MIN_ERROR_VALUE || value > MAX_ERROR_VALUE)
    ...
    for (const auto &arg : args)
    {
        std::string cmd = std::string(AT_CMD_CLEAR_ERROR) + AT_CMD_EQUAL + arg + AT_CMD_CARRIAGE_RETURN;
        exec_cmds.push(cmd);
    }
...
const std::string ClearErrorReqHandler::prepareCommand(void)
{
...
bool ClearErrorReqHandler::reqExecEnded(void)
{
    if (this->exec_cmds.empty())
    {
