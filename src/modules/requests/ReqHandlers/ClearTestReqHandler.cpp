...
ArgValidationResult ClearTestReqHandler::validateArgs(const std::vector<std::string> &args)
{
    if (args.empty())
    ...
    for (const auto &arg : args)
    {
        std::string cmd = std::string(AT_CMD_CLEAR_TEST) + AT_CMD_EQUAL + arg + AT_CMD_CARRIAGE_RETURN;
        exec_cmds.push(cmd);
    }
...
const std::string ClearTestReqHandler::prepareCommand(void)
{
...
bool ClearTestReqHandler::reqExecEnded(void)
{
    if (this->exec_cmds.empty())
    {
